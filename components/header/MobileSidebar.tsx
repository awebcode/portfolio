...
import Paragraph from "../reusables/contents/elements/Paragraph";
...
const DropdownItem: React.FC<{ item: MenuItem; depthLevel: number }> = ({
    ...
    const paddingLeft = `${depthLevel * 4}px`;
            ...
            >
                <NavigationMenu>
                    <NavigationMenuItem>  <NavigationMenuTrigger >Getting started</NavigationMenuTrigger>
                        <NavigationMenuContent className="p-4 w-40"> {nestedSidebarMenuData.map((item) => <Link key={item.label} href={item.href||"#"}> <Paragraph >{item.label}</Paragraph></Link>)}</NavigationMenuContent></NavigationMenuItem>
                </NavigationMenu>
